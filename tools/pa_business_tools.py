...
TGG_CASE_QUERY_SCHEMA = {
    ...
    "description": (
        ...
        "receipt_date, canonical_state, physical_state, administrative_state, "
        "is_terminal, is_submitted_for_closure, current_comment, "
        "current_comment_source, next_action, hdb_job_status, "
        "is_hdb_outstanding, hdb_urgency_label, hdb_work_costing_status, "
        "hdb_proof_of_completion, master_status, master_comment, "
        "sfc_submission_state, whatsapp_account_state, "
        "has_accepted_whatsapp, reconciliation_run_id, "
        "reconciliation_policy_version, reconciled_at, reconciliation_as_of); "
        "tgg_case_sources(job_no, source_account_id, source_kind, "
        "acceptance_state, snapshot_key, native_locator, artifact_hash, "
        "source_effective_at, captured_at, account_effective_at, "
        "account_observed_at, facts_json, provenance_json, unknowns_json); "
        "tgg_case_reconciliation(job_no, reconciliation_run_id, "
        "policy_version, field_decisions_json, disagreements_json, "
        "unknowns_json, validation_state, created_at); "
        "tgg_case_current_work_items(job_no, work_item_id, label, state, "
        "explanation, ordinal, evidence_refs_json, effective_at, "
        "whatsapp_source_account_id). field_decisions_json is the authoritative "
        "derivation receipt for why a canonical field was selected; do not infer "
        "that reason from sources that merely agree with the result. Count accepted "
        "WhatsApp case accounts with tgg_case_truth.has_accepted_whatsapp and current "
        "WhatsApp-derived work items with tgg_case_current_work_items separately. "
        "For dated HDB cohorts, receipt_date is HDB Job Creation Date/Time. "
        "For a single known case normally use "
        "tgg_case_lookup; for finding a case by address/unit normally use "
        "tgg_case_search. Legacy schema (SQLite; timestamps are "
