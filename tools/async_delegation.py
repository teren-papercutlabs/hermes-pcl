...
try:  # Python 3.14 tracks executor queues for interpreter shutdown.
...
class _DaemonThreadPoolExecutor(ThreadPoolExecutor):
    ...
    def _worker_args(self, executor_ref):
        """Return private ``_worker`` args across Python versions.

        Python 3.14 changed ThreadPoolExecutor internals from
        ``(executor_ref, work_queue, initializer, initargs)`` to
        ``(executor_ref, worker_context, work_queue)``. Keep the daemon
        executor compatible with both shapes.
        """
