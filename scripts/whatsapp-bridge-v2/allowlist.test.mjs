...
test('matchesAllowedChat is fail-closed and accepts explicit chat ids', () => {
  const sessionDir = mkdtempSync(path.join(os.tmpdir(), 'hermes-wa-outbound-'));

  try {
    const allowedChats = parseIdentifierList('120363001234567890@g.us, 15551234567@s.whatsapp.net');
    assert.equal(matchesAllowedChat('120363001234567890@g.us', allowedChats, sessionDir), true);
    assert.equal(matchesAllowedChat('120363009999999999@g.us', allowedChats, sessionDir), false);
    assert.equal(matchesAllowedChat('15551234567:12@s.whatsapp.net', allowedChats, sessionDir), true);
    assert.equal(matchesAllowedChat('120363001234567890@g.us', parseIdentifierList(''), sessionDir), false);
    assert.equal(matchesAllowedChat('120363001234567890@g.us', null, sessionDir), false);
  } finally {
    rmSync(sessionDir, { recursive: true, force: true });
  }
});
