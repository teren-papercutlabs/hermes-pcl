...
ELIGIBLE_RETENTION_STATES = ("complete", "bypassed")
...
def plan(inbox: Path, cutoff_seq: int) -> dict[str, Any]:
    with _connect(inbox, read_only=True) as conn:
    ...
    *,
    ...
    try:
        ...
        if after_work_selection["at_or_before_cutoff"] != 0:
            raise CutoffError("post-mutation work selection still includes cutoff rows")
        if after_status.get("pending", 0) != before_status.get("pending", 0) - changed:
            raise CutoffError("pending count did not fall by the selected count")
        if after_status.get("skipped", 0) != before_status.get("skipped", 0) + changed:
...
def restore_cutoff(
    inbox: Path, before_image: Path, *, confirm_run_id: str
) -> dict[str, Any]:
    ...
    try:
        ...
        if run is None:
            raise CutoffError(f"cutoff run is not recorded in the inbox: {run_id}")
        if str(run["before_image_sha256"]) != _artifact_sha256(before_image):
            raise CutoffError("before-image SHA-256 does not match the inbox audit row")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except (CutoffError, json.JSONDecodeError, OSError, sqlite3.Error) as exc:
